...
def login_to_atos_portal():
    ...
    # Fill DAS ID
    browser_fill_credential(selector="input[placeholder*='DAS']", credential_env="ATOS_USERNAME")

    # Fill Password
    browser_fill_credential(selector="input[type='password']", credential_env="ATOS_PASSWORD")

    # Check if AUTH Code is required (2FA)
    if "AUTH Code" in page_content:
        print("⚠ 2FA AUTH Code required. Please enter it manually.")
        browser_wait_for_user(message="Please enter your AUTH Code and click AUTH to continue.", timeout_seconds=120)
    else:
        # Submit the form
