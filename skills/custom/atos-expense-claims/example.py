...
def navigate_to_expense_claims():
    ...
    if "MyAtos" not in page_content.get("title", "") and "Page d'accueil" not in page_content.get("body", ""):
    ...
    if "My expenses" in expense_page.get("body", "") or "Poste de travail personne en déplacement" in expense_page.get(
        "body", ""
    ):
...
def navigate_to_expense_claims_with_error_handling():
    ...
    try:
        ...
        try:
            ...
            return {"status": "error", "error": str(e), "message": "Could not find or click 'Note de frais' menu"}
        ...
        is_correct_page = (
            "My expenses" in expense_page.get("body", "")
            or "Poste de travail personne en déplacement" in expense_page.get("body", "")
            or "Expense-claim" in expense_page.get("url", "")
        )
        ...
        print(f"\n❌ ERROR: {e}")
        return {"status": "error", "error": str(e), "message": "Navigation failed"}
