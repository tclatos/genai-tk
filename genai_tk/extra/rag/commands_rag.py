...
def _print_add_files_resolution(console: Console, params: dict[str, Any]) -> None:
    """Render the effective ``rag add-files`` parameters."""
    table = Table(title="RAG Add-Files Parameters", show_header=True, header_style="bold cyan")
...
class RagCommands(CliTopCommand):
    ...
    def register_sub_commands(self, cli_app: typer.Typer) -> None:  # noqa: C901, PLR0912, PLR0915
        ...
        def add_files(
            base_dir: Annotated[
                str,
                typer.Argument(help="Root directory to walk (supports ${paths.*} config vars)"),
            ],
            pathspec: Annotated[
                Optional[list[str]],
                typer.Option("--pathspec", "-p", help="Gitwildmatch pattern (repeatable; prefix ! to exclude)"),
            ] = None,
            ...
            dry_run: Annotated[
                bool, typer.Option("--dry-run", help="Show resolved parameters without executing")
            ] = False,
            ...
            _print_add_files_resolution(console, params)
            if dry_run:
